...
half_clone_conversation() {
    local source_session="$1"
    local project_path="$2"
    local keep_percent="${3:-50}"

    local clone_timestamp
    clone_timestamp=$(date "+%b %-d %H:%M" 2>/dev/null) || clone_timestamp=$(date "+%b %e %H:%M" | sed 's/  / /')
    ...
    keep_clean_count=$(( (total_clean_user_messages * keep_percent + 99) / 100 ))
    local skip_clean_count
    skip_clean_count=$((total_clean_user_messages - keep_clean_count))
    ...
    if [ -n "$clean_user_lines" ]; then
        ...
        if echo "$last_line_info" | grep -qE '<command-message>(dx:)?clone</command-message>|<command-message>(dx:)?half-clone</command-message>|<command-message>clone</command-message>|<command-message>clone-half</command-message>' 2>/dev/null; then
    ...
    if [ "$last_clone_cmd_line" -gt 0 ] && [ "$last_clone_cmd_line" -eq "$last_clean_user_line" ]; then
        ...
        log_info "Will exclude /clone-half command and subsequent messages (line $stop_at_line onwards)"
    fi
...
while [ $# -gt 0 ]; do
    case "${1:-}" in
        ...
        --keep-percent)
            shift
            KEEP_PERCENT="${1:-50}"
            if ! [[ "$KEEP_PERCENT" =~ ^[0-9]+$ ]] || [ "$KEEP_PERCENT" -lt 10 ] || [ "$KEEP_PERCENT" -gt 90 ]; then
                log_error "Invalid keep-percent value: $KEEP_PERCENT. Must be an integer between 10 and 90."
                exit 1
            fi
            shift
            ;;
...
if [ "$PREVIEW_MODE" = true ]; then
    ...
    half_clone_conversation "$SESSION_ID" "$PROJECT_PATH" "$KEEP_PERCENT"
fi
