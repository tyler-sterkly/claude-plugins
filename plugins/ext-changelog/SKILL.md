...
Generate the commit title and body as part of changelog output. These are internal
developer-facing artifacts. NOT subject to the changelog redaction rules. You can use:
- Version numbers
...
### Handing off to sys-commit

After generating the commit title and body, do NOT run `git commit` directly.
Instead invoke sys-commit in invoked mode, passing:
- `title` — the generated commit title string
- `body` — the generated commit body string
- `repo_path` — the project root path
- `already_committed` — false

sys-commit handles user approval and the actual `git commit`. ext-changelog's job ends
when it passes these values. Do not push after sys-commit returns.
